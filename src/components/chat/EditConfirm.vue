...
import { ref } from 'vue'
import type { EditConfirmData, ScheduleQueryItem } from '../../types'
...
<template>
  <div class="edit-confirm">
    ...
    <template v-if="isDone()">
      <div v-if="data.userAction === 'editing'" class="flex items-center gap-2 p-3 bg-blue-50 rounded-xl border border-blue-200">
        <i class="fa-solid fa-pen-to-square text-blue-500"></i>
        <span class="text-sm text-blue-700 font-medium">已打开编辑</span>
      </div>
      <div v-else-if="data.userAction === 'skipped'" class="flex items-center gap-2 p-3 bg-gray-50 rounded-xl border border-gray-200">
        <i class="fa-solid fa-forward text-gray-500"></i>
        <span class="text-sm text-gray-600">已跳过</span>
      </div>
    </template>
    ...
    <template v-else>
      ...
      <div v-if="data.matchedSchedule" class="space-y-3">
        <div class="text-xs text-gray-500 mb-1">
          <i class="fa-solid fa-crosshairs mr-1"></i> 识别到以下日程，是否修改？
        </div>

        <div class="p-3 bg-white rounded-xl border-2 border-blue-200 shadow-sm">
          ...
          <div class="flex gap-2 mt-3">
            <button
              ...
              class="flex-1 px-3 py-2 bg-blue-500 text-white text-xs font-medium rounded-lg hover:bg-blue-600 transition"
            >
              <i class="fa-solid fa-pen-to-square mr-1"></i> 编辑此日程
            </button>
            <button
              @click="showAll = true"
      ...
      <div v-if="!data.matchedSchedule || showAll" class="mt-3">
        <div class="text-xs text-gray-500 mb-2">
          ...
          {{ data.matchedSchedule ? '请选择要修改的日程：' : '未找到匹配日程，请从列表选择：' }}
        </div>
        ...
        <div class="space-y-1.5 max-h-64 overflow-y-auto">
          <div
            ...
            class="flex items-center gap-3 p-2.5 bg-white rounded-xl border border-gray-100 hover:border-blue-300 hover:shadow-md cursor-pointer transition group"
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm font-bold text-gray-800 truncate group-hover:text-blue-600 transition">
                {{ item.content }}
              </div>
            ...
            <div class="text-gray-300 group-hover:text-blue-400 transition">
